...
def img_callback(image, args):
    ...
    if method == "detection":
    ...
    elif method == "crop_classification":
        crops_bytes_to_send = MODEL.send_crops(frame_copy)
        if len(crops_bytes_to_send) > 0:
            input_frame = gabriel_pb2.InputFrame()
            input_frame.payload_type = gabriel_pb2.PayloadType.VIDEO
            input_frame.payloads.extend(crops_bytes_to_send)
            ...
            rospy.loginfo(
                "Sent image msg with size {:.2f} KB and crops {}".format(
                    len(serialized_message) / 1024, len(crops_bytes_to_send)
                )
            )
        ...
        send_flag = MODEL.send(frame_copy)
        ...
        if send_flag == True:
            ...
            rospy.loginfo(
                "Sent image msg with size {:.2f} KB".format(
                    len(serialized_message) / 1024
                )
            )
...
if __name__ == "__main__":
    ...
    try:
        run_node("camera" + str(args.camera_id), "delphi_" + args.target_name)
